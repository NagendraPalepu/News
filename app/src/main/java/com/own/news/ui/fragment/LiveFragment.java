...
import com.own.news.ui.ChannelPlaybackActivity;
...
public class LiveFragment extends Fragment {
    ...
    private void getNewChannelsData () {
        GetDataTask getDataTask = new GetDataTask (context, "livetv.json");
    ...
    class ChannelsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        ChannelsAdapter (ArrayList<NewsSourceResponse.Channels> albumList, String type) {
        ...
        public void onBindViewHolder (@NonNull final RecyclerView.ViewHolder holder, int position) {
            ...
            if (holder instanceof ChannelsAdapter.MyViewHolder) {
                NewsSourceResponse.Channels channels = albumList.get (holder.getAdapterPosition ());

                if (channels != null) {
                    ImageUtils.loadImage (channels.getImageUrl (), ((ChannelsAdapter.MyViewHolder) holder).thumbnail, ImageUtils.getRequestOptions (R.drawable.ic_launcher_background));
                    ((ChannelsAdapter.MyViewHolder) holder).title.setText (channels.getName ());


                    ((ChannelsAdapter.MyViewHolder) holder).layout.setOnClickListener (new View.OnClickListener () {
                        @Override
            ...
            } else if (holder instanceof ChannelsAdapter.MyViewHolder2) {
                NewsSourceResponse.Channels channels = albumList.get (holder.getAdapterPosition ());

                if (channels != null) {
                    ImageUtils.loadImage (channels.getImageUrl (), ((ChannelsAdapter.MyViewHolder2) holder).thumbnail, ImageUtils.getRequestOptions (R.drawable.ic_launcher_background));
                    ((ChannelsAdapter.MyViewHolder2) holder).title.setText (channels.getName ());

                    ((ChannelsAdapter.MyViewHolder2) holder).layout.setOnClickListener (new View.OnClickListener () {
                        @Override
                        public void onClick (View v) {
                            Intent intent = new Intent (context, ChannelPlaybackActivity.class);
                            intent.putExtra ("Object", albumList.get (holder.getAdapterPosition ()));
                            startActivity (intent);
                        }
                    });
